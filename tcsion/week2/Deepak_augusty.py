...
def series(ip: int) -> float:
    """
    Calculate the series sum based n  on the given integer ip and return the result.
    ...
    if(ip < 0):
        ...
        sumOfNum=float(0)
        for n in range(1,ip+1):
            sumOfNum = sumOfNum + (frac(n)/(n*n))
        return round(sumOfNum,2)
    ## CODEBASE:S01 End
